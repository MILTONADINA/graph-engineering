...
- `backend`: `express`. The registry has no `fastify` or `nestjs` nodes, so do not select them for a `generate` action.
...
- `deployment`: `docker` by default (the registry's CI/build-verification story). The registry has no `serverless` node, so do not select it.
- `frontend`: `none` unless requirements explicitly ask for a UI. Select `nextjs` for a Next.js UI or `react` for a client-rendered Vite UI. Both root/API-client pairs are implemented. The existing authentication, form, table and dashboard frontend nodes require the Next.js client; report those integration gaps for React rather than selecting incompatible nodes.
...
1. Always start with `project.node-express` (order 0).
   If the stack includes a frontend, add its separate root/client pair in dependency order: `project.nextjs` then `frontend.nextjs` for Next.js, or `project.vite-react` then `frontend.react` for React. Do not put both frontend roots in one application target.
...
4. Order the final list topologically over the `requires` edges. If a cycle remains, stop and report it rather than guessing an order; the shipped registry should never contain one.
