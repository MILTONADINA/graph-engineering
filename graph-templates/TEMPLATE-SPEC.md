...
- **`id`** is the primary key across the whole system — the registry, artifacts, and other templates' `dependencies.templates`/`compatible_with` all reference it. Renaming an `id` is a breaking change (§7).
...
Every project generated by this system SHOULD maintain a `.graph/manifest.json` at its root recording `{ nodeId: { version, generatedAt, files: [...] } }` — this is what `detect` and `version-check` read. Generating this manifest is itself the responsibility of `project/node-express` (or any `project/*` node) as part of its own `generate`.
