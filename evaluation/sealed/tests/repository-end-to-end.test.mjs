...
test(
  "native repository attempt closes through original-byte audit and signed vault aggregate without authority",
  ...
  async (t) => {
    ...
    // The other arm is synthetic bookkeeping; the candidate arm below is a
    // real persisted one-call invocation of the native repository guest.
    ...
    const requestRef = await artifacts.put(requestBytes);
    requestBytes.fill(0);
    ...
    const responseRef = await artifacts.put(
      Buffer.from(
        canonicalJson({
          model: "weights-v1",
    ...
    const call = store.reserveCall(candidate.reservationId, {
      callId: "call-candidate",
      providerId: "local-worker",
      requestedModel: "weights-v1",
      requestSha256: requestRef.sha256,
      reservedCostUsd: 0,
    });
    const callReceipt = store.completeCall({
      version: "1.0.0",
    ...
    candidateReceipt.observations[0].callId = call.callId;
