...
Python additionally has a bounded isolated CPython `ast`/`symtable` pass when a trusted local interpreter is available. It resolves direct lexical/import bindings without executing or importing repository code, and records the actual interpreter version and intermediate-source provenance. Unsupported runtimes and dynamic cases stay explicit fallbacks. See [Python coverage and process limits](context-lifecycle.md#isolated-python-bindings), including the sampled—not instantaneous—macOS memory watchdog. Go adds a fixed packaged `go/types` helper for direct functions and represented module/workspace imports; it compiles only that helper, never repository code, and retains complete package/config/dependency provenance. See [Go coverage and process limits](context-lifecycle.md#snapshot-only-go-bindings). Java adds fixed `javac` snapshot analysis with no annotation processors or target execution; C# adds SDK8 Roslyn analysis without MSBuild, generators or target execution. See [Java](context-lifecycle.md#snapshot-only-java-bindings) and [C#](context-lifecycle.md#snapshot-only-c-bindings) for exact subsets, project restrictions and resource limits. Rust adds pinned rust-analyzer LSP declaration navigation through an isolated edition-2021 source snapshot, with no Cargo or target execution. Its verified runtime is currently Linux-fixture-only, not provisioned on Mac/Windows; see [Rust subset and limits](context-lifecycle.md#snapshot-only-rust-bindings). None of these passes establishes complete multi-language runtime semantics.
...
Use `--client local` only for a consumer whose inference actually stays local. Cloud-backed clients are refused for offline projects. Cloud graph lookups apply the export allowlist to seeds and both resolved endpoints before traversal, so non-exportable files cannot disclose target identifiers or connect otherwise separate public nodes. MCP exposes context, symbols, graph relationships, templates, memory proposals, and run status, list and events; cloud clients see run status and list only when the server runs with `--allow-run-status`, their `run_list` omits objectives, and their `run_events` carries each event's type, time and step without its data. `--allow-run` separately enables `plan_decompose` (a planner proposes steps; [proposed decomposition](decomposition.md)), `plan_create` (acceptance criteria required), `run_start` and `run_cancel`, so a connected AI can break work down, plan, start, follow (`run_events` until `complete`) and stop a run. A cloud-backed client can decompose and create plans only while publication is `none`, and its planner sees only exportable context. A plan that publishes (commit or draft PR) cannot be started over MCP or the dashboard API until a person reviews it with `graph-engine plan-approve <planId>` and approves it with `--yes`, an approval bound to the plan's exact content; `graph-engine run` counts as the person's approval ([plan approval spec](../specs/integration/plan-approval.md)). Resuming a stopped run needs a person to acknowledge its retained state (`graph-engine resume --reconciled` or the dashboard), so MCP offers no resume. MCP does not replace a native client's own permissions or intercept all its model calls.
...
To enable cloud API workers, review `.graph/project.json`: its checked-in inference/network mode is already `allowlisted`, but only local `qwen`/`laya` providers are permitted and no outbound hosts are listed. Add explicitly reviewed cloud provider IDs and exact HTTPS hosts, and review the selected exportable file patterns before use. Do not use wildcard network hosts. A strict cost budget requires known pricing and a compatible API worker; missing telemetry is unknown, not zero.
...
Verification resolves local image tags to immutable image IDs and records that identity. Detailed failure excerpts stay local; cloud/native workers receive generic check-failure feedback because logs may contain non-exportable source. They may request additional explicitly exportable files, whole or as `path#L<start>-L<end>` line ranges; a file too large to send whole returns an outline of its symbols and line ranges ([worker context excerpts](worker-context-excerpts.md)). This trades some debugging convenience for an enforceable export boundary.
...
All 53 implemented catalog nodes have audited deterministic proposal renderers, including backend/CRUD, full-text search, webhook ingress, authentication/authorization, database lifecycle, storage, project/frontend scaffolds, documentation, testing and DevOps. They validate schemas, package/export prerequisites, bounded values, exact shared-file edits and generated paths, then propose artifacts for sandbox verification. They do not install dependencies or run template hooks on the host. The ECS Express Mode composer emits only an offline plan and private request, never deploys. Public examples and root ledgers require [explicit policy permissions](public-template-artifacts.md); private memory and engine control state remain protected. The three planned nodes remain unavailable. See the [coverage inventory and security limits](dag-and-template-runtime.md#remaining-catalog-inventory); capability is not deployment, provider compatibility or human acceptance.

`plan OBJECTIVE --accept CRITERION --steps reviewed-steps.json` accepts an explicit dependency DAG with per-step workers or supported templates. Ready proposals run concurrently within policy capacity; patch application is serialized, overlapping independent write sets are rejected, and checkpoints bind plan/policy/workspace identity. Pending applications require reconciliation after interruption. The service also reserves project-wide worker slots and inference budget. New run and worker leases bind to a process-instance proof, so a reused PID cannot validate them. Legacy PID-only leases retain a live PID conservatively during an in-place upgrade; if that PID was reused, an operator must reconcile the lease manually. This is a bounded scheduler, not automatic decomposition of every engineering request into a proven architecture. See [DAG and template runtime](dag-and-template-runtime.md).
