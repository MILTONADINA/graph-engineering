...
Cost-capped Jev calls require reviewed fixed-unit pricing or a reviewed input-token price with a full 64k-token pre-dispatch reservation, plus a persistent ledger. Without known pricing or ledger capacity, they abstain. Every dispatched batch has one call identity; its cost is not counted once per question. Token-priced calls settle from reported input usage; missing usage leaves the reservation unresolved. Unknown usage, legacy untracked calls, and unresolved charges remain visible rather than becoming zero. The ledger includes planning-only and failed calls as well as completed runs. These are conservative client-side estimates and reported charges, not a provider-enforced financial guarantee. See [decision accounting](decisions.md#jev-accounting-and-budgets).
...
All 48 implemented catalog nodes have audited deterministic proposal renderers, including backend/CRUD, full-text search, webhook ingress, authentication/authorization, database lifecycle, storage, project/frontend scaffolds, documentation, testing and DevOps. They validate schemas, package/export prerequisites, bounded values, exact shared-file edits and generated paths, then propose artifacts for sandbox verification. They do not install dependencies or run template hooks on the host. The ECS Express Mode composer emits only an offline plan and private request, never deploys. Public examples and root ledgers require [explicit policy permissions](public-template-artifacts.md); private memory and engine control state remain protected. The seven planned nodes remain unavailable. See the [coverage inventory and security limits](dag-and-template-runtime.md#remaining-catalog-inventory); capability is not deployment, provider compatibility or human acceptance.
