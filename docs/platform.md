...
Twenty-one fine-grained nodes have audited deterministic proposal renderers: the nine backend foundation nodes, `database.transactions`, composed `api.crud`, four documentation nodes, five testing nodes, and `devops.environments`. They validate input/output schemas, package/export prerequisites, bounded field expressions, exact shared-file edits and generated paths, then propose artifacts for sandbox verification. They do not install dependencies or run template commands/hooks on the host. Environment examples require explicit policy permission; private memory and credentials remain excluded. The other 21 implemented and 13 planned nodes remain unavailable through this runtime; see the [explicit coverage inventory](dag-and-template-runtime.md#remaining-catalog-inventory) and application-security limitations.
