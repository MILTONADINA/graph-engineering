...
All 52 implemented catalog nodes have audited deterministic proposal renderers, including backend/CRUD, full-text search, webhook ingress, authentication/authorization, database lifecycle, storage, project/frontend scaffolds, documentation, testing and DevOps. They validate schemas, package/export prerequisites, bounded values, exact shared-file edits and generated paths, then propose artifacts for sandbox verification. They do not install dependencies or run template hooks on the host. The ECS Express Mode composer emits only an offline plan and private request, never deploys. Public examples and root ledgers require [explicit policy permissions](public-template-artifacts.md); private memory and engine control state remain protected. The four planned nodes remain unavailable. See the [coverage inventory and security limits](dag-and-template-runtime.md#remaining-catalog-inventory); capability is not deployment, provider compatibility or human acceptance.
