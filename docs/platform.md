...
The compiler runs in a separate worker using only indexed, hash-validated source in an in-memory host. It does not load repository `tsconfig`, plugins, libraries, `node_modules`, package metadata, or network content. Snapshot identity includes parser/compiler versions; cross-file bindings are recomputed after source changes, including changes to an otherwise cached caller's dependencies. Package/path aliases, dynamic imports and dispatch, arbitrary object methods, overloaded or reassigned callables, ambiguous re-exports, and implicit cross-file script globals are not statically resolved. Limits are 1,000 TS/JS files, 8 MiB source, 250,000 AST nodes, a 5-second worker deadline, and a 256-MiB old-generation heap limit. Exceeding a limit retains syntax/heuristic evidence and reports a coverage diagnostic; it does not claim complete semantic coverage. An unchanged fallback snapshot retains its diagnostic instead of silently retrying expensive analysis. `--lexical` skips embeddings, not this bounded source analysis.
