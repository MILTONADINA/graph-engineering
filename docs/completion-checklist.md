...
| 10  | Dependency scheduling and fine-template execution          | Validated DAG, per-step providers, coordinated parallel proposals, shared limits and durable checkpoints implemented. Ten audited renderers cover backend foundations and transactions; 32 implemented and 13 planned catalog nodes remain unavailable. See the explicit runtime inventory.           |
| 11  | Memory freshness/conflicts and semantic graph              | Bounded compiler-backed JS/TS static bindings now supplement syntax/heuristic evidence. Reviewed typed claims support exact scoped conflicts, normalized import and guarded supersession; text-only conflicts remain heuristic. No complete runtime graph or automatic truth adjudication is claimed. |
...
Remaining implementation coverage is explicit in the
[template inventory](dag-and-template-runtime.md#remaining-catalog-inventory)
and [context lifecycle](context-lifecycle.md). Static JS/TS resolution does not
cover dynamic dispatch, project aliases, external packages, or compiler-backed
bindings for the other languages. [Typed memory assertions](memory-assertions.md)
