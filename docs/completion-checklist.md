...
| 11  | Memory freshness/conflicts and semantic graph              | Bounded JS/TS, Python, Go, Java and C# declaration bindings preserve source/config privacy, with real isolated compiler evidence. Missing native runtimes explicitly fall back. Rust integration and dynamic/whole-program semantics remain incomplete. Reviewed memory claims do not automatically adjudicate truth.                              |
...
and [context lifecycle](context-lifecycle.md). Static TS/JS, Python, Go, Java and C# resolution
does not cover arbitrary dynamic dispatch, external installed packages, or
Rust semantic bindings. [Typed memory assertions](memory-assertions.md)
