...
| 3   | Real workers and verification commands                     | Local Qwen configured; repository verification image provisioned with dependency-metadata checks and network-disabled execution. The Graph worker adapter passed a live, no-change structured call against the existing oMLX Qwen endpoint. The separate sealed one-call relay still has only native Docker/fake-loopback tests. Paid workers await provider/budget selection.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| 5   | MCP client integration                                     | Real stdio tool discovery and cloud-filtered retrieval passed. Cloud `context_get` now defaults to lexical retrieval, with explicit graph/hybrid opt-in. A constrained Claude Code Max-session call returned exportable source through the project MCP; normal project-wide approval remains separate. Codex recognizes the server; Cursor connection remains unverified.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
