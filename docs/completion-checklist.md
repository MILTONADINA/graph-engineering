...
| 13  | Representative calibration and held-out evidence           | Nine pinned historical intake tasks and four additional bounded known-history replays are implemented. Sealed bookkeeping, exact public-packet retention, a one-call local model relay, narrow digest and one-file QuickJS engineering oracles, vault-backed original-byte audit, full-cohort accounting, purpose-separated signed aggregate inspection, and a non-authorizing current-witness comparison exist. Native engineering tests use a fake model response and synthetic private cases. **Not complete as evidence:** general protected repository tests, authenticated worker/oracle provenance and external witness, independent unseen tasks/labels, and paired measured model costs/outcomes remain required. |
...
- Implement separately governed sealed held-out collection/execution: frozen
  ...
  one-file QuickJS engineering verifier have native tests. The engineering
  claim binds a retained local model response, response-derived patch, result
  source and private case verdict in one ledger. The aggregate inspector
  re-derives those joins from original bytes, but the relay tests use a fake
  loopback model and the private cases are synthetic. A separate current-witness
  ...
  General repository-test execution, authentic worker/oracle transport and an
