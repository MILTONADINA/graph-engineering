...
| 13  | Representative calibration and held-out evidence           | Nine pinned historical intake tasks and four additional bounded known-history replays are implemented. Guarded candidate checks, sealed bookkeeping and paid-call reservations, exact public-packet retention, a public-only one-shot Docker intake, original-byte audit, full-cohort accounting and a non-authorizing promotion preflight exist. **Not complete as evidence:** protected model/oracle execution, signed independent reviews, unseen held-out tasks and paired measured model costs/outcomes remain required. |
...
- Implement separately governed sealed held-out collection/execution: frozen
  ...
  public-only one-shot Docker intake, post-closure byte audit, paid spending
  cap and complete-cohort accounting are implemented and tested. The intake
  acknowledges packet content without running a model or seeing an oracle. A
  claim or unsigned acknowledgment is not delivery evidence, a protected model
  worker, or protected oracle access. Existing known-history tasks cannot be
  relabeled as unseen.
