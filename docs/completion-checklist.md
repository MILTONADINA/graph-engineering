...
| 5   | MCP client integration                                     | Real stdio discovery and cloud-filtered retrieval passed; cloud `context_get` defaults to lexical retrieval. Claude Code Max retrieved exportable source through the project MCP. Codex `context_get` returned a live export-filtered packet after a fresh local index. Cursor Serena activation is confirmed under project name `GRAPH ENGINEERING`; the separate `graph-engineering` project MCP server is discovered but disconnected pending its own in-app enablement and read-only tool check.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 6   | Real end-to-end engineering run                            | Qwen + Laya + Docker synthetic smoke, assisted full-repository verification and one unassisted retrospective zero-budget replay passed. A real cloud-export privacy defect was reproduced, fixed and verified in this branch; a live Claude Max managed worker proposed a change from selected public source without applying it. A fresh, unassisted full engineering run on that defect and human/partner acceptance remain separate.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
