...
| 3   | Real workers and verification commands                     | Local Qwen configured; repository verification image provisioned with dependency-metadata checks and network-disabled execution. The Graph worker adapter passed a live, no-change structured call against the existing oMLX Qwen endpoint. The sealed v2 two-arm local cohort also completed once against existing oMLX Qwen with pinned Docker images and retained private-oracle verdicts on a known synthetic fixture; it is not held-out evidence. Paid workers await provider/budget selection.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| 5   | MCP client integration                                     | Real stdio tool discovery and cloud-filtered retrieval passed. Cloud `context_get` now defaults to lexical retrieval, with explicit graph/hybrid opt-in. A constrained Claude Code Max-session call returned exportable source through the project MCP; normal project MCP approval was completed without approving other servers. Codex recognizes the server; Cursor connection remains unverified.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| 10  | Dependency scheduling and fine-template execution          | Validated DAG, shared budgets and durable checkpoints implemented. All 43 implemented catalog nodes now have audited renderers, with strict generated-code, real PostgreSQL, offline SDK, Docker and browser evidence. Twelve planned catalog stubs remain unavailable. Deployment/provider compatibility and human acceptance are separate.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
