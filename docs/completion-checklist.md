...
- Implement separately governed sealed held-out collection/execution: frozen
  ...
  answers only an exact-byte question. A bounded private collector can inspect
  original bytes and purpose-separated row/aggregate signatures, but its
  artifact bytes and trust pins are caller-supplied; it grants no authority.
