...
| Tester        | A second worker (`tester`), when configured                                                      | After the developers' steps, writes tests that prove each acceptance criterion, changing only test files; later repair steps may not edit the tests it wrote |
...
| Tests for every criterion             | The tester step writes tests for each acceptance criterion; repairs cannot weaken them. Writing them before implementation is a planned improvement                                                                                                                                   | `graph-engine tester <provider-id>`                                        |
