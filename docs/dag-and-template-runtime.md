...
All 52 implemented catalog graph nodes now have audited deterministic proposal renderers, subject to explicit prerequisites and project policy. The four planned nodes remain unavailable. A registry-wide test checks every implemented identity and exact manifest against its registered renderer; capability alone is not proof of deployment readiness.
