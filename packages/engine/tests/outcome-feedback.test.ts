...
it("retains one external claim without routing and rejects settled-usage mismatch", () => {
  ...
  try {
    ...
    const meta = {
      projectId: "project-a",
      ownerId: "GRAPH-42/handoff/1",
      binding,
      policyVersion: "p".repeat(64),
    ...
    const observation = (provider: "laya" | "jev") => {
      ...
      const usage = {
        ...
        inputTokens: 10,
        outputTokens: 2,
        reportedCostUsd: provider === "jev" ? 0.1 : 0,
        estimatedCostUsd: provider === "jev" ? 0.1 : 0,
        chargedUsd: provider === "jev" ? 0.1 : 0,
        reservedUsd: provider === "jev" ? 0.1 : 0,
      ...
      store.settleCall(meta.ownerId, callId, provider, {
        inputTokens: 10,
        outputTokens: 2,
        cachedTokens: 0,
      ...
      return {
        ...
        configuredModel: `${provider}-v1`,
        observedModel: `${provider}-v1`,
        callId,
        choices: { dispatch: "proceed" },
        records: [
          {
    ...
    const dual: DualConsultEvidence = {
      version: "1.0.0",
      projectId: "project-a",
    ...
    const plan: ExecutionPlan = {
      ...
      policyHash: meta.policyVersion,
      createdAt: "2026-09-23T00:00:00.000Z",
      objective: "Selected task",
      acceptance: ["Selected task passes"],
      steps: [
        {
          id: "step",
          kind: "worker",
    ...
    store.event(run.id, "publication.completed", {
      commit: null,
      pullRequest: null,
    });
    ...
    const feedback = {
      ...
      handoff: "handoff",
      dispatch_id: meta.ownerId,
      source_before_sha256: binding.sourceSha256,
      candidate_sha256: "b".repeat(64),
      run_id: run.id,
      scope_sha256: scopeSha256,
      workspace_snapshot_sha256: snapshotHash,
      scope_id: "scope-42",
      scope_request_sha256: "c".repeat(64),
      completion_proof_sha256: "d".repeat(64),
      dual_consultation_sha256: createHash("sha256")
        .update(bytes)
        .digest("hex"),
      outcome: {
        kind: "BRIGHTPATH_COMPLETION_REVIEW",
        state: "CURRENT_ENGINEERING_COMPLETION",
      ...
      decision_usage: {
        laya: dual.observations.laya.usage,
        jev: dual.observations.jev.usage,
      },
      memory_accepted: false,
      routing_promoted: false,
    };
    const db = new Database(path.join(directory, "runs.sqlite"));
    try {
      db.prepare("UPDATE inference_calls SET usage_json=? WHERE id=?").run(
        JSON.stringify({
          inputTokens: 10,
          outputTokens: 2,
          cachedTokens: 0,
      ...
      expect(() => store.recordOutcomeFeedback(feedback, bytes)).toThrow(
        "Settled decision usage differs",
      );
      db.prepare("UPDATE inference_calls SET usage_json=? WHERE id=?").run(
        JSON.stringify({
          inputTokens: 10,
    ...
    expect(() =>
      store.recordOutcomeFeedback(
        { ...feedback, candidate_sha256: "2".repeat(64) },
        bytes,
      ),
    ).toThrow("different reviewed outcome");
    expect(() =>
      store.recordOutcomeFeedback(
        { ...feedback, dual_consultation_sha256: "3".repeat(64) },
        bytes,
      ),
    ).toThrow("bytes changed");
    expect(() =>
      store.recordOutcomeFeedback(
        { ...feedback, scope_sha256: "4".repeat(64) },
        bytes,
      ),
    ).toThrow("not bound to the retained dual task and scope");
    expect(() =>
      store.recordOutcomeFeedback(
        { ...feedback, workspace_snapshot_sha256: "5".repeat(64) },
        bytes,
      ),
    ).toThrow("snapshot is not the retained verified GE run");
    expect(store.outcomeSummary()).toMatchObject({
      unverifiedClaims: 1,
      kind: "advisory-ge-run-observations",
      observedRuns: 1,
      groups: [
        { provider: "jev", observed: 1, automatedPassed: 1 },
        { provider: "laya", observed: 1, automatedPassed: 1 },
      ],
      localDecisionContextEligible: true,
