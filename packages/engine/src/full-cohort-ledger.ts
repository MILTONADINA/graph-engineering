...
export function validateFullCohortLedger(
  ...
  for (const item of inspection.assignments) {
    ...
    if (oracleInvocation) {
      ...
      require(same(oracleInvocation, {
        ...
        "sealed-call-bound-repository-v2-invocation-claim"
          ? {
              scopeSha256: oracleInvocation.scopeSha256,
              baselineTreeSha256: oracleInvocation.baselineTreeSha256,
              recipeSha256: oracleInvocation.recipeSha256,
              resultSourceFormat: "sealed-repository-execution-tree-v2",
            }
          : {}),
      ...
      if (
        ...
        require(task.stateFormatVersion === "repo-snapshot-v1" &&
          task.executionScopeSha256 === oracleInvocation.scopeSha256 &&
          oracleInvocation.resultSourceSha256 !==
            oracleInvocation.proposalSha256 &&
          ![
            task.baselineSha256,
            task.publicPacketSha256,
            task.oracleSha256,
            oracleInvocation.scopeSha256,
            oracleInvocation.resultSourceSha256,
          ].includes(oracleInvocation.recipeSha256) &&
          ![
            task.baselineSha256,
            task.publicPacketSha256,
            task.oracleSha256,
            ...
            oracleInvocation.baselineTreeSha256,
          ), "repository v2 claim differs from frozen scope or execution tree roles");
    ...
    if (
      ...
      require(receipt.status !== "completed" &&
        receipt.outcome.success ===
          null, "call-bound oracle cannot authorize measured attempt success");
    ...
    if (
      ...
      require((receipt.proposalSha256 === null ||
        ...
        (receipt.resultSourceSha256 === null ||
          receipt.resultSourceSha256 === oracleInvocation.resultSourceSha256) &&
        (receipt.outcome.verificationSha256 === null ||
          receipt.outcome.verificationSha256 ===
            oracleVerdict?.verificationSha256), "engineering attempt settlement differs from private claim/verdict");
    if (oracleVerdict)
  ...
  for (const [index, item] of ordered.entries()) {
    ...
    if (
      ...
      const eventType =
        boundClaim.kind === "sealed-call-bound-engineering-invocation-claim"
          ? "call-bound-engineering-invocation-claimed"
          : boundClaim.kind ===
              "sealed-call-bound-module-graph-invocation-claim"
            ? "call-bound-module-graph-invocation-claimed"
