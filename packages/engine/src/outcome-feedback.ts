...
export const outcomeFeedbackSchema = z
  .object({
    ...
    outcome: z
      .object({
        kind: z.enum([
          "BRIGHTPATH_COMPLETION_REVIEW",
          "BRIGHTPATH_SCOPE_REVIEW",
        ]),
        state: z.enum([
          "CURRENT_ENGINEERING_COMPLETION",
          "REVIEW_REJECTED",
  ...
  .superRefine((value, context) => {
    if (
      ...
      (!value.completion_proof_sha256 ||
        value.outcome.kind !== "BRIGHTPATH_COMPLETION_REVIEW")
    )
      context.addIssue({
        code: "custom",
        message: "Completion requires a bound BrightPath completion proof",
      });
    if (
      value.outcome.state === "SCOPE_REVIEW_REJECTED" &&
      value.outcome.kind !== "BRIGHTPATH_SCOPE_REVIEW"
    )
      context.addIssue({
        code: "custom",
        message: "Scope rejection requires a scope review",
      });
  });
...
export function validateOutcomeFeedback(
  ...
  if (
    createHash("sha256").update(consultationBytes).digest("hex") !==
    feedback.dual_consultation_sha256
  )
    throw new Error("Retained consultation bytes changed");
  let supplied: unknown;
  try {
    supplied = JSON.parse(consultationBytes.toString("utf8"));
  } catch {
    throw new Error("Retained consultation is invalid JSON");
  }
  ...
  if (
    ...
    !retained.observations.laya.valid ||
    !retained.observations.jev.valid
  )
  ...
  if (
    ...
    outcomeHash(feedback.decision_usage) !==
      outcomeHash({
        laya: observations.laya.usage,
        jev: observations.jev.usage,
      })
  )
    throw new Error(
      "Feedback decision IDs or usage differ from retained observations",
    );
  ...
  if (
    ...
    throw new Error(
      "Feedback run is not bound to the retained dual task and scope",
    );
  if (
    run.id !== feedback.run_id ||
