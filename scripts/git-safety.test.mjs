...
test("post-update verification rejects a same-count check substitution and weakened rules", () => {
  ...
  const unexpectedLastPushReview = liveProtection("dev");
  unexpectedLastPushReview.required_pull_request_reviews.require_last_push_approval = true;
  assert.throws(
    () => assertProtectionMatches("dev", unexpectedLastPushReview),
    /verification failed/,
  );
